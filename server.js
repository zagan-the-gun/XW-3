...
// 拡張はこの文字列でそのまま選択する。サイト側の変更時はここだけ直せばよい。
//
// other: サイト側に自由入力の余地があるか。
//   false = 選択肢が確定している項目(UIでも自由入力を出さない)
//   true  = 一覧が網羅できていない可能性がある項目(「その他(自由入力)」を出す)
const CHOICES = {
  mercari: {
    condition: {
      other: false,
    ...
    shippingPayer: { other: false, options: ['送料込み(出品者負担)', '着払い(購入者負担)'] },
    // 「その他」配下のサービスが多く、表記を網羅できていない可能性があるため開けておく
    shipping: {
      other: true,
      options: [
    ...
    shipDays: { other: false, options: ['1~2日で発送', '2~3日で発送', '4~7日で発送'] },
    shipFrom: { other: false, options: PREFECTURES },
  },
  yahoo: {
    // Yahoo!フリマは5段階で「新品、」が付かない。配送料の負担は項目自体がない
    condition: {
      other: false,
      options: ['未使用', '未使用に近い', '目立った傷や汚れなし', 'やや傷や汚れあり', '傷や汚れあり'],
    },
    // ヘルプには「おまかせ配送」もあるが出品フォームに出るか未確認のため開けておく
    shipping: { other: true, options: ['おてがる配送（ヤマト運輸）', 'おてがる配送（日本郵便）'] },
    shipDays: { other: false, options: ['1~2日', '2~3日', '3~7日'] },
    shipFrom: { other: false, options: PREFECTURES },
  },
};
